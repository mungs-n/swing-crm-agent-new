  ...
  return (
    <ResponsiveContainer width="100%" height={height}>
      <ScatterChart margin={{ top: 10, right: 20, bottom: 10, left: 0 }}>
        ...
        <YAxis type="number" dataKey="monetary" name="구매 금액" tick={{ fontSize: 11, fill: "#888699" }} tickFormatter={(v) => `${(v / 10000).toFixed(0)}만`} />
        <ZAxis type="number" dataKey="monetary" range={[40, 300]} />
        <Tooltip
          cursor={{ strokeDasharray: "3 3" }}
          formatter={(value, name) => (name === "구매 금액" ? [`₩${value.toLocaleString()}`, name] : [value, name])}
          labelFormatter={() => ""}
        />
